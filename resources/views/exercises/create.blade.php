...
<div class="container mx-auto px-2 sm:px-4 py-4 sm:py-8">
    ...
    <h1 class="text-3xl font-bold text-gray-800 mb-6">Create New Exercise</h1>
    
    <form action="{{ route('exercises.store') }}" method="POST" class="bg-white shadow-md rounded-lg p-6">
        @csrf
        ...
        <div class="mb-6">
            ...
            <textarea name="description" id="description" rows="3"
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                placeholder="A short summary of what this exercise teaches...">{{ old('description') }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            <textarea name="instructions" id="instructions" rows="8"
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                placeholder="Step-by-step instructions for performing this exercise...">{{ old('instructions') }}</textarea>
